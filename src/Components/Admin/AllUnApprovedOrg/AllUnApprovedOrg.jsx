...
const AllUnApprovedOrg = () => {
    const [fetchAgain,setFetchAgain]=useState(true);
   const {getAllUnapprovedOrganizations,
    getAllApprovedOrganizations,
    approveOrganizationAccount,
    disapproveOrganizationAccount}=useContext(context);
    ...
    const fetchDetailsOfUnApproved=async()=>{
        try {
            let response=await getAllUnapprovedOrganizations();
    ...
    const fetchDetailsOfApproved=async()=>{
        try {
            let response=await getAllApprovedOrganizations();
            if(response.data.status==="success"){
    ...
    const approveIt=async(id)=>{
        try {
            let response=await approveOrganizationAccount(id);
            if(response.data.status==="success"){
    ...
    const disapproveIt=async(id)=>{
        try {
            let response=await disapproveOrganizationAccount(id);
            if(response.data.status==="success"){
                toast.success(response.data.message);
                setFetchAgain(!fetchAgain)
            }
    ...
    return (
      <div className="container mx-auto p-4">
        <h1 className="text-3xl font-bold mb-4">Organization Id Approval</h1>
        {/* Stats Section */}
        <div className="mb-8 border rounded-lg p-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p>Approved Organizations: {approved.length}</p>
            </div>
            <div>
              <p>Unapproved Organizations: {pending.length}</p>
            </div>
          </div>
        </div>
                ...
                <tbody>
                  {pending.map(volunteer => (
                    <tr key={volunteer._id} className="bg-white">
                      <td className="border px-4 py-2">{volunteer.organizationName}</td>
                      <td className="border px-4 py-2">{volunteer.organizationEmail}</td>
                     
                      <td className="border px-4 py-2">
                        <Link to={`/profileOfOrganization/${volunteer._id}`}>
...
export default AllUnApprovedOrg