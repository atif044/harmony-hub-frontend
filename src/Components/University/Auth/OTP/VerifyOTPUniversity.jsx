...
const VerifyOTPUniversity = () => {
  const [otp, setOtp] = useState('');
  const navigate=useNavigate();
  const {verificationOTPUniversity,resendOTPUniversity,setisVerifed}=useContext(context);
  ...
  const handleVerify = async(event) => {
    ...
    try {
        ...
        let response=await verificationOTPUniversity(otp);
  ...
  const handleResend =async () => {
    try {
        ...
        let response=await resendOTPUniversity();
        if(response.data.status==="success"){
            return toast.success(response.data?.message);
        }
...
export default VerifyOTPUniversity;
